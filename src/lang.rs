...
fn c_output(
    ...
    let version_string = version.fully_qualified_string(repo_path)?;
    let content = format!(
...
pub fn output_file(
    ...
    match lang {
        Language::C => c_output(version, path, repo_path),
        Language::Go => go_output(version, path, repo_path),
        Language::Java => java_output(version, path, repo_path),
        Language::CSharp => csharp_output(version, path, repo_path),
        Language::Python => python_output(version, path, repo_path),
    }
}
