...
fn main() {
    let matches = Command::new("semver-bumper")
        .arg(Arg::new("major")
            .long("major")
        ...
        .arg(Arg::new("patch")
            .long("patch")
            .action(clap::ArgAction::SetTrue)
            .conflicts_with_all(&["major", "minor"])
    ...
    // Get the output file path, which has a default value of "version.h"
    let out_file = matches.get_one::<String>("output-file").unwrap();
