...
"#,
    ...
    fn bump(&mut self, major: bool, minor: bool, patch: bool) -> Result<(), BumpError> {
        if !major && !minor && !patch {
            return Err(BumpError::LogicError(
                "Nothing to bump! Must provide one of --major, --minor, or --patch".to_string(),
            ));
        }
        if major {
            self.major += 1;
...
fn main() -> ExitCode {
    ...
    if matches.get_flag("init") {
    ...
    } else if matches.get_flag("major") || matches.get_flag("minor") || matches.get_flag("patch") {
        // If any bump flag is set, proceed to bump the version
        let mut version = match get_version(&matches) {
            ...
            Err(err) => {
                if let BumpError::IoError(_) = err {
                    ...
                    eprintln!("File not found: {}", resolved_path.display());
        ...
        if let Some(output_file) = matches.get_one::<String>("output-file") {
            if let Err(err) = fs::write(output_file, version.to_header()) {
